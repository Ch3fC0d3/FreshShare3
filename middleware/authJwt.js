...
const DEBUG_AUTH_LOGGING = NODE_ENV !== 'production' || process.env.AUTH_DEBUG === 'true';
const authLog = (...args) => {
  if (DEBUG_AUTH_LOGGING) {
    console.log(...args);
  }
};
...
const verifyToken = (req, res, next) => {
  ...
  try {
    authLog(`[authJwt] ${req.method} ${req.originalUrl}`);
    authLog('[authJwt] has cookie-parser:', !!req.cookies);
    authLog('[authJwt] cookies.token:', maskToken(cookieToken));
    authLog('[authJwt] Authorization header:', formatAuthHeader(authHeaderValue));
    authLog('[authJwt] Referer:', req.headers && req.headers.referer ? req.headers.referer : '(none)');
    authLog('[authJwt] Origin:', req.headers && req.headers.origin ? req.headers.origin : '(none)');
  } catch (logErr) {
    try { authLog('[authJwt] logging error:', logErr); } catch (_) {}
  }
  
  // Check cookies first (preferred method for web pages)
  if (cookieToken) {
    token = cookieToken;
    authLog(`Token found in cookies for ${req.method} ${req.originalUrl}`);
  } 
  // Then check authorization header (for API calls)
  else if (authHeaderValue) {
    const authHeader = authHeaderValue;
    authLog(`Raw Authorization header: ${authHeader}`);
    
    // IMPORTANT: Always extract token properly regardless of format
    if (authHeader.startsWith('Bearer ')) {
      token = authHeader.substring(7).trim();
      authLog('Bearer prefix found, extracted token after prefix');
    } else {
      token = authHeader.trim();
      authLog('No Bearer prefix found, using header value as is');
    }
    
    authLog(`Token found in Authorization header for ${req.method} ${req.originalUrl}`);
    authLog(`Token extracted (first 15 chars): ${token.substring(0, 15)}...`);
    authLog(`Token length: ${token.length}`);
    authLog(`JWT Secret first 5 chars: ${JWT_SECRET.substring(0, 5)}...`);
    ...
    try {
      ...
      if (decoded && decoded.id) {
        ...
        authLog(`Set token cookie from Authorization header for user ID: ${decoded.id}`);
      }
    } catch (err) {
      authLog('Error verifying token from Authorization header:', err && err.message);
  ...
  // Finally check x-access-token (legacy support)
    ...
    authLog(`Token found in x-access-token header for ${req.method} ${req.originalUrl}`);
    ...
    try {
      ...
      if (decoded && decoded.id) {
        ...
        authLog(`Set token cookie from x-access-token header for user ID: ${decoded.id}`);
      }
    } catch (err) {
  ...
  if (!token) {
    authLog(`No token found for ${req.method} ${req.originalUrl}`);
  ...
  let decoded = null;
  try {
    authLog('Attempting to verify token with primary JWT secret');
    authLog('Token length:', tokenValue.length);
    authLog('Token first 15 chars:', tokenValue.substring(0, 15) + '...');
    authLog('JWT_SECRET first 10 chars:', JWT_SECRET.substring(0, 10) + '...');
    
    decoded = jwt.verify(tokenValue, JWT_SECRET);
    authLog('Token successfully verified with primary JWT secret');
  } catch (primaryError) {
    authLog('Primary token verification failed:', primaryError && primaryError.message);
    authLog('Primary error name:', primaryError && primaryError.name);
    
    try {
      if (NODE_ENV !== 'production') {
        authLog('Attempting to verify token with legacy JWT secret');
        authLog('LEGACY_JWT_SECRET first 10 chars:', LEGACY_JWT_SECRET.substring(0, 10) + '...');
      }
      
      decoded = jwt.verify(tokenValue, LEGACY_JWT_SECRET);
      if (NODE_ENV !== 'production') {
        authLog('Token verified using legacy JWT secret. Consider reissuing tokens.');
        authLog('Decoded token user ID (legacy):', decoded.id);
      }
    } catch (legacyError) {
      authLog('Legacy token verification also failed:', legacyError && legacyError.message);
      authLog('Legacy error name:', legacyError && legacyError.name);
  ...
  try {
    ...
    authLog('Token post-verification processing failed:', error && error.message);
    if (req.originalUrl.startsWith('/api/')) {
      return res.status(401).json({
        success: false,
        message: 'Unauthorized! Token is invalid or expired.'
